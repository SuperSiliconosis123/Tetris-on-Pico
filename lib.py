...
DC = 8
RST = 12# Raspberry Pi Pico microcontroller 
# WaveShare Pico LCD 1.3 inch 240x240 screen with Joystick and Buttons
# Import the libraries needed
from machine import Pin,SPI,PWM
import framebuf
...
class tetris():
    ...
    def initStats(level):
        ...
        printstring("SCORE: " + '{:0>{w}}'.format(str(tetris.score), w=5), 130, 5, 1, 65535)
        printstring("LINES: " + '{:0>{w}}'.format(str(tetris.lines), w=4), 135, 15, 1, 65535)
...
class tetris():
    ...
    def initStats(level):
        ...
        lcd.fill_rect(5, 5, 110, 30, 0)
        printstring("TETRIS", 5, 10, 2, 65535)
        printstring("SCORE: " + '{:0>{w}}'.format(str(score), w=5), 130, 5, 1, 65535)
        printstring("LINES: " + '{:0>{w}}'.format(str(lines), w=4), 135, 15, 1, 65535)
        printstring("LEVEL: " + str(level), 148, 25, 1, 65535)
        lcd.fill_rect(120, 40, 20, 10, 65535)# skew
        lcd.fill_rect(130, 50, 20, 10, 65535)
        lcd.fill_rect(130, 70, 20, 10, 65535)# inverted skew
